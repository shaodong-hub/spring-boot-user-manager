...
public class LocalCachedMapConfig {
    ...
    @Bean
    public RLocalCachedMap<RequestMatcher, Collection<ConfigAttribute>> rLocalCachedMap(RedissonClient redisson) {
        return redisson.getLocalCachedMap("test", options());
    }

    private LocalCachedMapOptions<RequestMatcher, Collection<ConfigAttribute>> options() {
        return LocalCachedMapOptions.<RequestMatcher, Collection<ConfigAttribute>>defaults()
                // 用于淘汰清除本地缓存内的元素
                // 共有以下几种选择:
